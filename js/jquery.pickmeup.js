...
(function ($) {
	...
	var	views	= {
		...
		tpl		= {
			...
			body	: function (elements, container_class_name,date) {
				var result	= '';

				for (var i = 0; i < elements.length; ++i) {
					if(i<=9) text__="0"+(i-1);
					else text__ = (i-1);
					result	+= '<div class="' + elements[i].class_name + ' pmu-button '+ date+"-"+text__ +'">' + elements[i].text + '</div>'
				}
	...
	function fill () {
		var options			= $(this).data('pickmeup-options'),
			...
			current_cal		= /*Math.floor(options.calendars / 2)*/0,
		...
		for (var i = 0; i < options.calendars; i++) {
			...
			(function () {
				...
				__anio	= local_date.getFullYear();
				__month	= local_date.getMonth();
				if(__month<=9) __month = "0"+__month;

				__date = __anio+"-"+__month;

				first_month	= new Date(actual_date[0]).getMonth(),
				html	+= tpl.body(days, 'pmu-days',__date);
			})();
